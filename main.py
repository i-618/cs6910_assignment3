...
from sequenceModel import Encoder, Decoder, train
import torch
import os
...
decoder = Decoder(hidden_size=hidden_size, output_size=local_script.vocab_size, device=device).to(device)

loss = train(dataloader_train, encoder, decoder, 10, print_every=1, plot_every=1, device=device)
